...
- [ ] 4.1 Routes+repo: `sales/route.ts` M (paymentMethod/paidAmount server-side + pagination); `sales/[id]/returns/route.ts` C POST (409 over-qty, stock restore, audit); `receivables/route.ts` C GET; `receivables/[id]/pay/route.ts` C POST [S4a]
- [ ] 4.2 Routes: `suppliers/route.ts` M PATCH; `suppliers/[id]/route.ts` C PATCH/deactivate; `inventory/route.ts` M branchId-scoped update + negative-value guards; `inventory/[id]/route.ts` M DELETE (409 if referenced) [S4b]
