...
> **STATUS: ARCHIVED — 2026-09-14.** All 25/25 tasks complete (S1, S2, S3a–S3d, S4a–S4d, S5a/S5b). Verification: PASS WITH WARNINGS (16/16 specs, 18/18 requirements, 50/50 scenarios, 0 CRITICAL). Delta specs folded into `openspec/specs/` and this change folder moved to `openspec/changes/archive/2026-09-14-distribution-complete/` by sdd-archive. Final-state record: `archive-report.md` in the archived folder.
...
- [x] 4.4 Views: `SalesPOSView.tsx` M (tender/credit), `DistributionDashboard.tsx` M (drop hardcodes), `InventoryView.tsx` M (multi-branch/delete), `SalesHistoryView.tsx` M (pagination); i18n [S4d] — see Batch 10 in apply-progress
