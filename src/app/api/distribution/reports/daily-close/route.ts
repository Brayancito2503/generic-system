...
import { dailyCloseQuerySchema } from '@/core/schemas/distribution';
...
export async function GET(request: NextRequest) {
  try {
    ...
    const query = dailyCloseQuerySchema.safeParse(
      Object.fromEntries(request.nextUrl.searchParams.entries())
    );
    if (!query.success) throw new ApiError(400, 'Datos inválidos');

    const report = await repository.getDailyCloseReport(tenantId, query.data.date);
